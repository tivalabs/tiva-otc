...
// Type definitions for custom shader config would be ideal but keeping JS structure for now as requested
// We need to type the props for robustness if possible, but I'll stick to the provided code structure 
// while making it TSX compatible (e.g., adding types where implicit any might fail if strict)
...
function createStackedPlanesBufferGeometry(n: number, width: number, height: number, spacing: number, heightSegments: number) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        for (let j = 0; j <= heightSegments; j++) {
            // Shift geometry down by 10 units to center it lower on screen
            const y = height * (j / heightSegments - 0.5) - 10;
